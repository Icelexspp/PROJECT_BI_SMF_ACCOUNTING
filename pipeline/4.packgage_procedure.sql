create or replace PACKAGE                         DWH_SMF.SP_LOAD_ACT_BUD_ACC
AS
   PROCEDURE MAIN;

   PROCEDURE DELETE_BEFORE_LOAD_TBFACT_SMF_ACT_BUD_ACCOUNT;

   PROCEDURE LOAD_TBFACT_SMF_ACT_BUD_ACCOUNT;



END; 
/
create or replace PACKAGE BODY                      DWH_SMF.SP_LOAD_ACT_BUD_ACC
IS
   /*
   BEGIN
      DWH_SMF.SP_LOAD_ACT_BUD_ACC.MAIN;
   END;
   */
-----------------------------------------------------------------------------
   PROCEDURE DELETE_BEFORE_LOAD_TBFACT_SMF_ACT_BUD_ACCOUNT
   IS
   BEGIN
      EXECUTE IMMEDIATE 'TRUNCATE TABLE DWH_SMF.TBFACT_SMF_ACT_BUD_ACCOUNT';

      COMMIT;
   END DELETE_BEFORE_LOAD_TBFACT_SMF_ACT_BUD_ACCOUNT;
...
-----------------------------------------------------------------------------
   ...
   IS
   BEGIN
   INSERT INTO DWH_SMF.TBFACT_SMF_ACT_BUD_ACCOUNT
(
    DATA_SOURCE,
	JOBNO, 
	JOBSTAT,
...
FROM dwh_smf.tmep_smf_budget_account  TB;
      ...
      COMMIT;
   END LOAD_TBFACT_SMF_ACT_BUD_ACCOUNT;

 -------------------------------------------------------------------------------
   PROCEDURE MAIN --คนคุมสั่งการและเก็บลำดับว่าใครทำอะไรก่อนทำอะไรหลัง
   IS
   BEGIN
      DELETE_BEFORE_LOAD_TBFACT_SMF_ACT_BUD_ACCOUNT;

      LOAD_TBFACT_SMF_ACT_BUD_ACCOUNT;
